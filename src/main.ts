...
export default class ScreenShot {
  ...
  private load(
    ...
    this.data.setScreenShotInfo(
      ...
      +window.getComputedStyle(document.body).height.split("px")[0]
    );
    // 设置截图容器位置
    this.data.setScreenShotPosition(this.position.left, this.position.top);
    // 设置截图图片存放容器宽高
    this.screenShotImageController.width = +window
      .getComputedStyle(document.body)
      .width.split("px")[0];
    this.screenShotImageController.height = +window
      .getComputedStyle(document.body)
      .height.split("px")[0];
    // 用户有传宽高则使用用户传进来的
    ...
    if (this.plugInParameters.getMediaStream()) {
      this.sendStream(
        this.plugInParameters.getMediaStream(),
        cancelCallback,
        triggerCallback
      );
      return;
