...
var PlugInParameters_PlugInParameters = /*#__PURE__*/function () {
  function PlugInParameters() {
    ...
    if (initStatus) {
      ...
      PlugInParameters_stream = null; // 初始化完成设置其值为false
...
var screenShotImageController = null; // 截图容器是否可滚动

var noScrollStatus = false; // 数据初始化标识
...
var InitData_InitData = /*#__PURE__*/function () {
  ...
  _createClass(InitData, [{
    ...
    value: function setScreenShotInfo(width, height) {
      ...
      if (InitData_screenShotController == null) return; // 增加截图锁屏
    ...
    value: function destroyDOM() {
      ...
      var plugInParameters = new PlugInParameters_PlugInParameters(); // 销毁dom
      ...
      document.body.removeChild(cutBoxSizeContainer); // 重置插件全局参数状态
...
function getCanvas2dCtx(canvas, width, height) {
  ...
  var ctx = canvas.getContext("2d"); // 对画布进行缩放处理
...
function drawCutOutBox(mouseX, mouseY, width, height, context, borderSize, controller, imageController) {
  ...
  var data = new PlugInParameters_PlugInParameters(); // 画布、图片不存在则return
  ...
  context.save();
  context.globalCompositeOperation = "destination-over"; // 图片尺寸使用canvas容器的css中的尺寸
...
function drawMasking(context, imgData) {
  ...
  var maxHeight = Math.max(window.innerHeight || 0, Math.max(document.body.scrollHeight, document.documentElement.scrollHeight), Math.max(document.body.offsetHeight, document.documentElement.offsetHeight), Math.max(document.body.clientHeight, document.documentElement.clientHeight)); // 清除画布

  context.clearRect(0, 0, maxWidth, maxHeight); // 屏幕截图存在且展示截图数据的状态为true则进行绘制
...
function drawMosaic(mouseX, mouseY, size, degreeOfBlur, context) {
  // 获取设备像素比
  var dpr = window.devicePixelRatio || 1; // 获取鼠标经过区域的图片像素信息

  var imgData = context.getImageData(mouseX * dpr, mouseY * dpr, size * dpr, size * dpr); // 获取图像宽高
...
var main_ScreenShot = /*#__PURE__*/function () {
  ...
  function ScreenShot(options) {
    ...
    _defineProperty(this, "sendStream", function (stream, cancelCallback, triggerCallback) {
      if (stream instanceof MediaStream) {
        ...
        setTimeout(function () {
          ...
          if (context == null) return; // 赋值截图区域canvas画布

          _this.screenShotCanvas = context; // 将获取到的屏幕截图绘制到图片容器里

          var imgContext = getCanvas2dCtx(_this.screenShotImageController, containerWidth, containerHeight); // 对webrtc源提供的图像宽高进行修复
          ...
          imgContext === null || imgContext === void 0 ? void 0 : imgContext.drawImage(_this.videoController, 0, 0, fixWidth, fixHeight); // 初始化截图容器

          _this.initScreenShot(undefined, context, _this.screenShotImageController); // 执行截图成功回调
    ...
    _defineProperty(this, "screenShot", function (cancelCallback, triggerCallback) {
      ...
      _this.startCapture(cancelCallback).then(function () {
        setTimeout(function () {
          ...
          var containerHeight = (_this$screenShotImage4 = _this.screenShotImageController) === null || _this$screenShotImage4 === void 0 ? void 0 : _this$screenShotImage4.height; // 用户有传宽高时，则使用用户的
    ...
    this.plugInParameters = new PlugInParameters_PlugInParameters(); // 提取options中的有用参数设置到全局参数中
    ...
    this.setGlobalParameter(); // 修改截图容器可滚动状态

    this.data.setNoScrollStatus(options === null || options === void 0 ? void 0 : options.noScroll); // 加载截图组件
  ...
  _createClass(ScreenShot, [{
    ...
    value: function load(triggerCallback, cancelCallback) {
      ...
      var canvasSize = this.plugInParameters.getCanvasSize(); // 设置截图区域canvas宽高

      this.data.setScreenShotInfo(+window.getComputedStyle(document.body).width.split("px")[0], +window.getComputedStyle(document.body).height.split("px")[0]); // 设置截图容器位置
      ...
      this.screenShotImageController.height = +window.getComputedStyle(document.body).height.split("px")[0]; // 用户有传宽高则使用用户传进来的
      ...
      if (context == null) return; // 显示截图区域容器
      ...
      if (!this.plugInParameters.getWebRtcStatus()) {
        ...
        return;
      } // 传入MediaStream时(前提是需要进入webRtc模式)
    ...
    value: function showToolBar() {
      ...
      var containerHeight = this.screenShotContainer.height; // 工具栏的位置超出截图容器时，调整工具栏位置防止超出
      ...
      if (this.getFullScreenStatus) {
        var _containerHeight = parseInt(this.screenShotContainer.style.height); // 重新计算工具栏的x轴位置
    ...
    value: function initScreenShot(triggerCallback, context, screenShotContainer) {
      ...
      (_this$screenShotConta3 = this.screenShotContainer) === null || _this$screenShotConta3 === void 0 ? void 0 : _this$screenShotConta3.addEventListener("mouseup", this.mouseUpEvent); // 是否初始化裁剪框
